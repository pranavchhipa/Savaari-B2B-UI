...
import { DEMO_SOURCE_CITIES_RAW, DEMO_AIRPORT_LIST_RAW } from '../demo/demo-data';
...
export class CityService {
  ...
  getSourceCities(tripType: string, subTripType: string): Observable<City[]> {
    if (environment.demoMode) return of(DEMO_SOURCE_CITIES_RAW.map((r: any) => toCity(r)));
  ...
  getAirportList(): Observable<City[]> {
    if (environment.demoMode) return of(DEMO_AIRPORT_LIST_RAW.map(r => toAirportListCity(r as any)));

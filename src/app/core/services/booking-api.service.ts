...
import { DEMO_BOOKINGS_LIST, buildDemoBookingCreateResponse, nextDemoBookingId } from '../demo/demo-data';
...
export class BookingApiService {
  ...
  createBooking(request: CreateBookingRequest): Observable<CreateBookingResponse> {
    if (environment.demoMode) {
      const id = nextDemoBookingId();
      const r = buildDemoBookingCreateResponse(id);
      return of({ ...r, bookingId: id, reservation_id: r.data.reservation_id } as any);
    }
  ...
  updateInvoicePayerInfo(bookingId: string, invoicePayer: string): Observable<unknown> {
    if (environment.demoMode) return of({ status: 'success' });
    const token: string = this.auth.getPartnerToken() ?? '';
  ...
  updateVasBooking(payload: {
    ...
    if (environment.demoMode) return of({ status: 'success', data: undefined } as VasUpdateResponse);
    const token: string = this.auth.getPartnerToken() ?? '';
  ...
  cancelBooking(
    ...
    if (environment.demoMode) return of({ status: 'cancelled', message: 'Booking cancelled successfully (demo)' } as CancelBookingResponse);
  ...
  getAllBookings(): Observable<BookingDetails[]> {
    if (environment.demoMode) {
      const d = DEMO_BOOKINGS_LIST.bookingDetails;
      const upcoming = (d.bookingUpcoming || []).map((b: any) => ({ ...b, _bucket: 'upcoming' }));
      const completed = (d.bookingCompleted || []).map((b: any) => ({ ...b, _bucket: 'completed' }));
      const cancelled = (d.bookingCancelled || []).map((b: any) => ({ ...b, _bucket: 'cancelled' }));
      return of([...upcoming, ...completed, ...cancelled] as BookingDetails[]);
    }
  ...
  getBookingDetails(bookingId: string): Observable<BookingDetails> {
    if (environment.demoMode) {
      const d = DEMO_BOOKINGS_LIST.bookingDetails;
      const all = [...(d.bookingUpcoming || []), ...(d.bookingCompleted || []), ...(d.bookingCancelled || [])];
      const found = all.find((b: any) => String(b.booking_id) === String(bookingId)) || all[0];
      return of(found as unknown as BookingDetails);
    }
    return this.api.b2bGet<any>('booking-details', {
      userEmail: this.auth.getUserEmail(),
      token: this.auth.getB2bToken(),

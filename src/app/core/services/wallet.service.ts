...
export class WalletService {
  ...
  createWallet(): Observable<boolean> {
    if (environment.demoMode) return of(true);
  ...
  loadBalance(): void {
    if (environment.demoMode) {
      this.balanceSubject.next(15000);
      this.walletStatusSubject.next({ balance: 15000, status: 'ACTIVE', walletId: 'DEMO-WALLET-9001' });
      return;
    }
  ...
  loadHistory(page = 1, limit = 50): void {
    if (environment.demoMode) {
      const txns: WalletTransaction[] = [
        { id: 'TXN001', date: new Date(Date.now() - 1 * 86400000), type: 'TOPUP',           amount:  10000, balanceAfter: 15000, description: 'Wallet Top-up via Razorpay',       referenceId: undefined, status: 'SUCCESS' },
        { id: 'TXN002', date: new Date(Date.now() - 3 * 86400000), type: 'BOOKING_PAYMENT', amount:  -1050, balanceAfter: 5000,  description: 'Advance for Booking #DEMO1001',  referenceId: 'DEMO1001', status: 'SUCCESS' },
        { id: 'TXN003', date: new Date(Date.now() - 6 * 86400000), type: 'TOPUP',           amount:  5000,  balanceAfter: 6050,  description: 'Wallet Top-up via Razorpay',       referenceId: undefined, status: 'SUCCESS' },
        { id: 'TXN004', date: new Date(Date.now() - 8 * 86400000), type: 'REFUND',          amount:  3200,  balanceAfter: 1050,  description: 'Refund for Booking #DEMO0850',   referenceId: 'DEMO0850', status: 'SUCCESS' },
      ];
  ...
  initiateTopUp(amount: number, _isRetry = false): Observable<TopUpInitiateResponse | null> {
    if (environment.demoMode) return of({ orderId: 'order_DEMO123', amount: amount * 100, currency: 'INR', razorpayKeyId: 'rzp_test_demo' });
  ...
  verifyTopUp(orderId: string, paymentId: string, signature: string, amount: number): Observable<boolean> {
    if (environment.demoMode) { this.addTopUp(amount, paymentId); return of(true); }
  ...
  createBookingOrder(amount: number): Observable<TopUpInitiateResponse | null> {
    if (environment.demoMode) return of({ orderId: 'order_DEMO456', amount: amount * 100, currency: 'INR', razorpayKeyId: 'rzp_test_demo' });
    if (!this.isWalletActive()) {
  ...
  payForBooking(bookingId: string, amount: number, paymentOption: 1 | 2 | 3 = 1): Observable<{ success: boolean; transactionId?: string }> {
    if (environment.demoMode) return of({ success: true, transactionId: `TXN-DEMO-${Date.now()}` });
    if (!this.isWalletActive()) {

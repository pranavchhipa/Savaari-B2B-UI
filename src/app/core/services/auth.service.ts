...
import { DEMO_LOGIN_RESPONSE, DEMO_PARTNER_TOKEN_RESPONSE, DEMO_B2B_TOKEN, DEMO_PARTNER_TOKEN } from '../demo/demo-data';
...
export class AuthService {
  ...
  login(email: string, password: string): Observable<UserProfile> {
    if (environment.demoMode) {
      const resp = DEMO_LOGIN_RESPONSE;
      this.clearUserScopedStorage();
      this.b2bToken = resp.token;
      this.user = resp.user as any;
      this.userGst = resp.userGst as any;
      localStorage.setItem('loginUserToken', resp.token);
      localStorage.setItem('loggedUserDetail', JSON.stringify(resp.user));
      localStorage.setItem('userGst', JSON.stringify(resp.userGst));
      this.partnerToken = DEMO_PARTNER_TOKEN;
      localStorage.setItem('SavaariToken', DEMO_PARTNER_TOKEN);
      return of(resp.user as any);
    }
  ...
  autoLogin(): Observable<UserProfile | null> {
    if (environment.demoMode) return of(this.user);
  ...
  forgotPassword(email: string): Observable<{ success: boolean; message: string }> {
    if (environment.demoMode) return of({ success: true, message: 'Demo mode: password reset email simulated.' });

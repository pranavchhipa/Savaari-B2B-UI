...
export class AccountSettingsComponent implements OnInit {
  ...
  private loadProfileFromAuth() {
    ...
    if (user) {
      this.profile = {
        ...
        state: (user as any).state || (user as any).agentstate || '',
  ...
  updateProfile() {
    ...
    const payload = {
      ...
      userPhone: this.sanitize(this.profile.phone) || this.sanitize(this.profile.mobile),
      userMobile: this.sanitize(this.profile.mobile) || this.sanitize(this.profile.phone),
      ...
      agentState: this.sanitize(this.profile.state) || this.sanitize(this.profile.city),
      agentcityId: 0,

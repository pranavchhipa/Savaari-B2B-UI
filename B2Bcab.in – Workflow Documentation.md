...
From the Home page, the agent can create a new booking by selecting one of the following booking types:
    ...
    Airport Trip – Two options are available: airport pickup or airport drop (default: Drop to Airport). The agent needs to enter the pickup address first, then the airport, and optionally switch trip type. Time picker uses 15-minute intervals.

**2\. Select Car (Car Listing Page)**

After entering trip details and clicking Explore Cabs, the agent sees the car listing page:
- Cars displayed with image, name, "or equivalent" tag, and feature pills (KMs, Seater, Bags, AC)
- Inclusions tab (default open): shows what's included in fare — dynamic from API (e.g., Fuel Charges, Driver Allowance, Toll/State tax, GST)
- Exclusions tab: shows extra charges on actuals — dynamic from API (e.g., Pay ₹X/km after Y km, Multiple pickups/drops)
- Facilities tab: AC, seating capacity
- T&C tab: trip-specific terms and conditions
- Price shown inclusive of GST, center-aligned with SELECT CAR button
- Modify button: opens modal with working city autocomplete, 15-min time picker, re-fetches availability on submit
...
For all booking types, the agent provides:
    ...
    Pickup address
    Drop address (One Way trips only — not shown for Local or Round Trip)
    Date & time
    Customer (guest) details

**4\. Price Display & Mark-up Application**
...
After entering trip details, agent selects one of 3 payment options:
1. **Pay any amount now** — Slider 25–100%; driver collects remaining balance
2. **Pay 25% now, rest auto-deducted** — 25% paid now; remaining 75% + 20% buffer auto-deducted from wallet 48 hours before trip. If wallet balance insufficient, booking auto-cancelled.
3. **Zero cash — full wallet** — Full amount + 20% buffer deducted from wallet 48 hours before trip; buffer refunded post-trip (Recommended)

GST Invoice option available on booking page — auto-applied if GST on file, else redirects to Account Settings.
